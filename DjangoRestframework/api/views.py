...
# from DjangoRestframework.lib.utils.json_response import success
# from lib.manage.imageProcess import *
# from DjangoRestframework.api.models import Image
# from DjangoRestframework.api.serializers import ImageSerializer
#
# from DjangoRestframework.lib.manage.imageProcess import get_new, imageResize, get_hist_dict, opera
import re
from rest_framework.viewsets import ModelViewSet
from rest_framework.views import APIView
...
class addSmoothSkin(APIView):
    def post(self, request):
        ...
        dict['filepath'] = path
        #dict['d'] = 5
        dict['sigmaColor'] = request.data.get('imputsigmaColor')
        #dict['sigmaSpace'] = request.data.get('imputsigmaColor')
        opera('SmoothSkin', dict)
...
class addManga(APIView):
    def post(self, request):
        serializer, path = getISP(request)

        dict = {}
        dict['filepath'] = path
        opera('MangaPaint', dict)

        return success(serializer.data)


class addMosaic(APIView):
    def post(self, request):
        serializer, path = getISP(request)
